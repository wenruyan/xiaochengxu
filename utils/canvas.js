import blog from '../pages/awrad-progress/awrad-progress.js'
export default{ 
  data: {
    percentage: '', //百分比
    animTime: '', // 动画执行时间
  ...
  options:{
    ...
    run(c, w, h) {
      ...
      that.data.ctx2.arc(w, h, w-5, -0.5 * Math.PI, num); //每个间隔绘制的弧度
      that.data.ctx2.setStrokeStyle(this.data.color);
      that.data.ctx2.setLineWidth("6");
      that.data.ctx2.setLineCap("round");
      that.data.ctx2.stroke();
      that.data.ctx2.beginPath();
      that.data.ctx2.setFontSize(14); //注意不要加引号
      that.data.ctx2.setFillStyle(this.data.color);
